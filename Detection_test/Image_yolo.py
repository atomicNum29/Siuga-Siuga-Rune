...
detect_time = {'inference_time' : f'{round(inference_time / 1000, 4)}s',
               'code_time' : f'{round(model_time, 4)}s'}
print(f"infer : {detect_time['inference_time']}, code : {detect_time['code_time']}")
...
if model_name in data:
    data[model_name].append(detect_time)
# 이미 있으면 결과 이어서 추가
else:
    data[model_name] = [detect_time]
